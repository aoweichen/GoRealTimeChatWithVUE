...
import (
	...
	"GoRealTimeChat/Internal/Dao/GroupDao"
...
// GroupHandler GroupHandler结构体定义了一个群组处理器
type GroupHandler struct {
}

var (
	groupDao       GroupDao.GroupDAO         // 创建GroupDAO对象
	messageService Services.ImMessageService // 创建ImMessageService对象
)

// GetGroupList GetGroupList方法用于获取群组列表
// 参数：
// - ctx: *gin.Context类型，表示HTTP请求的上下文
func (*GroupHandler) GetGroupList(ctx *gin.Context) {
...
// 参数：
// - ctx: *gin.Context类型，表示HTTP请求的上下文
func (*GroupHandler) CreateGroup(ctx *gin.Context) {
...
// 参数：
// - ctx: *gin.Context类型，表示HTTP请求的上下文
func (*GroupHandler) ApplyJoin(ctx *gin.Context) {
	id := ctx.MustGet("id")                          // 获取当前用户的ID
	err, person := BaseHandlerFuncs.GetPersonId(ctx) // 调用GetPersonId方法，获取当前用户信息
	if err != nil {
		Response.FailResponse(Enums.ParamError, "参数错误！").WriteTo(ctx) // 若获取用户信息失败，则返回参数错误响应给客户端
		return
	}
	var group IMGroups.ImGroups
	if result := Model.MYSQLDB.Model(&IMGroups.ImGroups{}).Where("id=?", person.ID).Find(&group); result.RowsAffected == 0 {
...
// 参数：
// - ctx: *gin.Context类型，表示HTTP请求的上下文
func (*GroupHandler) GetUsers(ctx *gin.Context) {
	err, person := BaseHandlerFuncs.GetPersonId(ctx) // 调用GetPersonId方法，获取当前用户信息
	if err != nil {
		Response.FailResponse(Enums.ParamError, "参数错误！").WriteTo(ctx) // 若获取用户信息失败，则返回参数错误响应给客户端
		return
...
// 参数：
// - ctx: *gin.Context类型，表示HTTP请求的上下文
func (*GroupHandler) Logout(ctx *gin.Context) {
	err, person := BaseHandlerFuncs.GetPersonId(ctx) // 调用GetPersonId方法，获取当前用户信息
	if err != nil {
		Response.FailResponse(Enums.ParamError, "参数错误！").WriteTo(ctx) // 若获取用户信息失败，则返回参数错误响应给客户端
		return
...
func (*GroupHandler) AddOrRemoveUser(ctx *gin.Context) {
	...
	name := ctx.MustGet("name") // 获取当前用户的名称
	var group ImGroups
	if result := Model.MYSQLDB.Model(&IMGroups.ImGroups{}).Where("id=?", params.GroupId).Find(&group); result.RowsAffected == 0 {
		Response.FailResponse(Enums.ParamError, "群聊不存在！").WriteTo(ctx) // 若群聊不存在，则返回群聊不存在响应给客户端
		return
	}
	...
	Model.MYSQLDB.Model(&IMUser.ImUsers{}).
		Where("id in(?)", Model.MYSQLDB.Model(&IMGroupUsers.ImGroupUsers{}).
	...
	message := ApiRequests.PrivateMessageRequest{
		...
		MsgType:     Enums.JOIN_GROUP,    // 消息类型（加入群聊）
		Message:     "",                  // 消息内容
		SendTime:    Date.NewDate(),      // 发送时间
