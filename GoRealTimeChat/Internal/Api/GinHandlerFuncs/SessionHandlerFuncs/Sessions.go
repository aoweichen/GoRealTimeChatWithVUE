...
func (s SessionHandler) GetSessionsList(ginCtx *gin.Context) {
	...
	// 查询会话列表
	if result := Model.MYSQLDB.Model(&IMSessions.ImSessions{}).
...
func (s SessionHandler) AddSessions(ginCtx *gin.Context) {
	...
	if result := Model.MYSQLDB.Table("im_sessions").
		...
		var sessionsDao SessionDao.SessionDAO
...
// 置顶更新或者会话备注更新
func (s SessionHandler) UpdateSessions(ginCtx *gin.Context) {
	// 调用BaseHandlerFuncs.GetPersonId方法获取person信息，并返回错误和person
	...
	Model.MYSQLDB.Model(&IMSessions.ImSessions{}).Where("id", person.ID).Updates(&params)
...
func (s SessionHandler) DeleteSessions(ginCtx *gin.Context) {
	// 调用BaseHandlerFuncs.GetPersonId方法获取person信息，并返回错误和person
	err, person := BaseHandlerFuncs.GetPersonId(ginCtx)
	if err != nil {
		// 如果获取person失败，则记录错误日志并返回参数错误的响应给客户端
	...
	Model.MYSQLDB.Delete(&IMSessions.ImSessions{}, person.ID)
