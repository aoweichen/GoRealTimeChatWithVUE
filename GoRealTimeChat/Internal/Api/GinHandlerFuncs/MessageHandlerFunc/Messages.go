...
var (
	messageServices Services.ImMessageService
	friendDao       FriendDao.FriendDAO
	messageDao      MessageDao.MessageDao
)

func (MH *MessageHandler) sortByMessage(imMessageList []IMMessages.ImMessages, users IMUser.ImUsers) {
...
func (MH *MessageHandler) RecallMessage(ctx *gin.Context) {
...
func (MH *MessageHandler) GetPrivateChatList(ctx *gin.Context) {
	...
	// 构建查询条件，并按创建时间降序排序
	query := Model.MYSQLDB.Table("im_messages").
	...
	// 查询接收者信息
	Model.MYSQLDB.Table("im_users").Where("id=?", toID).First(&users)
...
// 参数:
// - ctx: gin.Context对象，用于处理HTTP请求和响应
func (MH *MessageHandler) SendVideoMessage(ctx *gin.Context) {
	// 定义变量users，用于存储IMUser.ImUsers对象
	var users IMUser.ImUsers
	...
	// 查询发送者信息
	Model.MYSQLDB.Table("im_users").Where("id=?", id).First(&users)
...
func (MH *MessageHandler) SendMessage(ctx *gin.Context) {
	...
	case 1:
		...
		Model.MYSQLDB.Model(&IMUser.ImUsers{}).Where("id=?", params.ToID).Find(&users)

		// 如果接收者是机器人用户
		if users.UserType == IMUser.BOT_TYPE {
