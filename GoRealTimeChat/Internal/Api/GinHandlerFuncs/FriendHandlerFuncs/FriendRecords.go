...
func (FRD *FriendRecordHandler) GetFriendsRequestRecord(ctx *gin.Context) {
	...
	if result := Model.MYSQLDB.Model(&IMFriendsRecords.ImFriendRecords{}).Preload("Users").
...
func (FRD *FriendRecordHandler) SendAddFriendsRequest(ctx *gin.Context) {
	// 从上下文中获取用户ID
	id := ctx.MustGet("id")
	...
	if result := Model.MYSQLDB.Table("im_users").
	...
	if result := Model.MYSQLDB.Table("im_friends").
	...
	Model.MYSQLDB.Save(&imFriendRecords)

	// 创建消息服务实例
	var messageService Services.ImMessageService
	// 创建好友消息对象
	message := MessageClient.CreateFriendMessage{
...
func (FRD *FriendRecordHandler) AgreeOrRejectFriendRequest(ctx *gin.Context) {
	// 定义变量
	var users IMUser.ImUsers                     // 用户信息
	var messageService Services.ImMessageService // 消息服务
	...
	// 从数据库中查询好友记录
	if result := Model.MYSQLDB.Table("im_friend_records").
	...
	// 检查好友关系是否已存在
	if result := Model.MYSQLDB.Table("im_friends").Where("from_id=? and to_id=?", records.FromId, records.ToId); result.RowsAffected > 0 {
		zap.S().Errorln("用户已经是好友关系了...")
		Response.ErrorResponse(Enums.ParamError, "用户已经是好友关系了...").ToJson(ctx)
	}

	// 更新好友记录的状态
	Model.MYSQLDB.Table("im_users").Where("id=?", id).Find(&users)
	records.Status = params.Status
	Model.MYSQLDB.Updates(&records)
...
// UserQuery 处理用户查询的函数
func (FRD *FriendRecordHandler) UserQuery(ctx *gin.Context) {
	// 定义变量
	var friendDao FriendDao.FriendDAO                                 // 声明一个 FriendDAO 类型的变量 friendDao
