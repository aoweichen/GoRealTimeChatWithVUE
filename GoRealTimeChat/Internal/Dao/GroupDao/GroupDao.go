...
func (*GroupDAO) GetGroupListJoinedByUser(userID interface{}) []IMGroups.ImGroups {
	// 使用Model.MYSQLDB从数据库中查询IMGroupUsers.ImGroupUsers表
	// 并将查询结果赋值给SQLQuery变量
	SQLQuery := Model.MYSQLDB.Model(&IMGroupUsers.ImGroupUsers{}).
		Where("user_id=?", userID).Select("group_id")

	var IMGroupLists []IMGroups.ImGroups

	// 如果查询结果中的行数大于0，则继续查询群组列表
	if result := Model.MYSQLDB.Model(&IMGroups.ImGroups{}).
...
func (*GroupDAO) GetGroupUsers(groupId string) []IMGroupUsers.ImGroupUsers {
	var groupUsers []IMGroupUsers.ImGroupUsers
...
func (*GroupDAO) CreateGroup(params ApiRequests.CreateGroupRequest) (error, IMGroups.ImGroups) {
	...
	// 在数据库中创建群组，并检查是否出现错误
	if Model.MYSQLDB.Model(&IMGroups.ImGroups{}).Create(&imGroups).Error != nil {
...
// - id: 成员ID
func (*GroupDAO) CreateOneGroupUser(group IMGroups.ImGroups, id int) {
	...
	// 在数据库中创建群组成员和会话
	Model.MYSQLDB.Model(&IMGroupUsers.ImGroupUsers{}).Create(&groupUser)
	Model.MYSQLDB.Model(&IMSessions.ImSessions{}).Create(&session)
...
func (*GroupDAO) DelSelectGroupUser(userIds []string, groupId int, avatar string, name string) {
	// 删除选定的群组成员
	Model.MYSQLDB.Model(&IMGroupUsers.ImGroupUsers{}).Where("user_id in(?)", userIds).Delete(&IMGroupUsers.ImGroupUsers{})

	// 删除相关会话
	Model.MYSQLDB.Model(&IMSessions.ImSessions{}).Where("group_id=? and from_id in(?)", groupId, userIds).Delete(&IMSessions.ImSessions{})
...
func (*GroupDAO) CreateSelectGroupUser(userIds []string, groupId int, avatar string, name string) {
	...
	// 在数据库中创建群组成员和会话
	Model.MYSQLDB.Model(&IMGroupUsers.ImGroupUsers{}).Create(&groupUser)
	Model.MYSQLDB.Model(&IMSessions.ImSessions{}).Create(&sessionsData)
