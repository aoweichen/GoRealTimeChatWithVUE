...
func (FD *FriendDAO) GetFriendLists(id interface{}) (error, interface{}) {
	var err error
	var friendList []IMFriends.ImFriends

	// 使用Model.DB从数据库中查询好友列表
	// 并将结果赋值给list变量
	result := Model.MYSQLDB.Model(&IMFriends.ImFriends{}).Preload("Users").
		Where("from_id=?", id).
		Order("status desc").
		Order("top_time desc").
		Find(&friendList)
...
func (FD *FriendDAO) AgreeFriendRequest(toID int64, fromID int64) {
...
// DelFriends 删除好友
func (FD *FriendDAO) DelFriends(toId interface{}, fromId interface{}) error {
	// 定义一个错误变量err
	var err error
...
// GetFriends 查询好友详情
func (FD *FriendDAO) GetFriends(id interface{}) (error, interface{}) {
	// 定义一个错误变量err
	var err error
...
func (f *FriendDAO) IsFriends(id interface{}, toId interface{}) bool {
	...
	Model.MYSQLDB.Table("im_friends").
