...
import (
	...
	"GoRealTimeChat/Packages/WebSocketServer"
...
func (*WSService) UserConnect(ctx *gin.Context) {
	...
	var dispatchService Dispatch.DispatchService                                                      // 创建 DispatchService 实例
	...
	wsClient := ClientManager.NewWSClient(Utils.Int64ToString(id), uid, userRole, conn) // 创建一个新的 WebSocket 客户端实例，使用 id 和 uid
	ClientManager.IMMessageClientManager.Register <- wsClient                           // 注册 WebSocket 客户端到客户端管理器
...
func (*WSService) TouristsConnect(ctx *gin.Context) {
	...
	wsClient := ClientManager.NewWSClient(id, "", touristsRole, conn) // 创建一个新的 WebSocket 客户端实例
	ClientManager.IMMessageClientManager.Register <- wsClient         // 注册 WebSocket 客户端到客户端管理器
