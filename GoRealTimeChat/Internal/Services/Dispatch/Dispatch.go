...
type DispatchServiceInterface interface {
	...
	// DeleteDispatch 删除当前节点
	DeleteDispatchNode(uid string)
}

// IsDispatchNode IsDispatch 判断当前节点是否分派服务节点
func (DS *DispatchService) IsDispatchNode(uid string) (bool, string) {
	// 调用redis.RedisDB的Exists方法，检查uid是否存在于Redis数据库中，结果赋值给numDispatchNode变量
	numDispatchNode, redisREDISDBExistsResultError := Redis.REDISDB.Exists(uid).Result()
	...
	if numDispatchNode > 0 {
		...
		if ConfigModels.ConfigData.Server.Node == uNode {
			return false, ""
		}
...
// GetDispatchNode 是 DispatchService 结构体的方法，用于获取调度节点
// 参数 uid 是用户 ID，表示要获取调度节点的用户
// 返回值是一个字符串，表示调度节点的信息
func (DS *DispatchService) GetDispatchNode(uid string) string {
	// 使用 Redis.REDISDB.Get 方法从 Redis 数据库中获取指定用户 ID 的值，并调用 Val 方法获取其字符串表示
	return Redis.REDISDB.Get(uid).Val()
}

// SetDispatchNode 设置分派节点
func (DS *DispatchService) SetDispatchNode(uid string) {
	// 调用Redis.REDISDB的Set方法，将uid作为key，ConfigModels.ConfigData.Server.Node作为value，设置过期时间为24小时
	Redis.REDISDB.Set(uid, ConfigModels.ConfigData.Server.Node, time.Hour*24)
}

// DeleteDispatchNode 是 DispatchService 结构体的方法，用于删除调度节点
// 参数 uid 是用户 ID，表示要删除的调度节点的用户
func (DS *DispatchService) DeleteDispatchNode(uid string) {
	// 使用 Redis.REDISDB.Del 方法从 Redis 数据库中删除指定用户 ID 的键值对
	Redis.REDISDB.Del(uid)
}

// MessageDispatch TODO 消息分发
func (DS *DispatchService) MessageDispatch() {
