...
import (
	...
	"GoRealTimeChat/Packages/MessageQueue"
...
func ConsumersPrivateMessageInit() {
	...
	if err := NSQQueue.NewConsumers(MessageQueue.OfflinePrivateTopic, ChannelOfflineTopic, ConfigModels.ConfigData.Nsq.LookupHost); err != nil {
		// 创建消费者失败，打印错误信息
...
func ConsumersGroupMessageInit() {
	...
	if err := NSQQueue.NewGroupConsumers(MessageQueue.OfflineGroupTopic, ChannelGroupOfflineTopic, ConfigModels.ConfigData.Nsq.LookupHost); err != nil {
		// 创建消费者失败，打印错误信息
		zap.S().Infoln("new nsq consumer failed", err)
		return
	}
...
func NodeInit() {
	...
	if err := NSQQueue.NewConsumers(MessageQueue.OfflinePrivateTopic, ChannelNodeTopic, ConfigModels.ConfigData.Nsq.LookupHost); err != nil {
		// 创建消费者失败，打印错误信息
		zap.S().Infoln("new nsq consumer failed", err)
		return
	}
