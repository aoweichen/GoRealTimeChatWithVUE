...
import (
	...
	"GoRealTimeChat/Internal/Services/Client/GRPCMessageClient"
	...
	"GoRealTimeChat/Packages/Cache/FriendCache"
	"GoRealTimeChat/Packages/CoroutinesPoll"
	"GoRealTimeChat/Packages/MessageOfflineDao"
	"encoding/json"
	"fmt"
...
func (Manager *IMClientManager) RadioUserOnlineStatus(client *IMClient) {
	// 从好友缓存中获取指定客户端的好友列表
	if imFriends, err := FriendCache.FriendCacheClients.GetFriendCache(client.ClientID); err == nil {
		// 遍历好友列表
...
// 该方法用于发送好友动作消息
func (Manager *IMClientManager) SendFriendActionMessage(message MessageClient.CreateFriendMessage) {
