...
var (
	messageHandler    MessageClient.MessageHandler
	gRPCMessageClient GRPCMessageClient.GRPCMessageService
...
func (IMC *IMClient) WSReadMessage() {
	...
	for {
		...
		if err != nil {
			...
			case MessageClient.GROUP:
				// 如果消息类型是群聊，则将确认消息通过客户端的连接发送回去
				_ = IMC.Socket.WriteMessage(websocket.TextMessage, ackMessage)
			case MessageClient.PING:
				// 如果消息类型是PING，则将确认消息通过客户端的连接发送回去
				_ = IMC.Socket.WriteMessage(websocket.TextMessage, ackMessage)
			case MessageClient.FORWARDING:
