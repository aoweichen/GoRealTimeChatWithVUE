...
func (OMD *OfflineMessageDao) PrivateOfflineMessageSave(message string) {
	...
	Model.MYSQLDB.Table("im_offline_messages").Create(&IMOfflineMessage.ImOfflineMessages{
		Status:    0,
...
func (OMD *OfflineMessageDao) PrivateOfflineMessageSaveOptimization(messages []string) {
	...
	Model.MYSQLDB.Table("im_offline_messages").Create(&offlineMessages)
	mutex.Unlock()
}
...
func (OMD *OfflineMessageDao) GroupOfflineMessageSave(message string) {
	...
	Model.MYSQLDB.Table("im_offline_messages").Create(&IMOfflineMessage.ImGroupOfflineMessages{
		Status:    0,
		SendTime:  int(Date.TimeUnix()),
		ReceiveId: userID,
		Message:   message,
