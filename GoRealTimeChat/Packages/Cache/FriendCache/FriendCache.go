...
var (
	FriendCacheClients = FriendCacheClient{
		CacheMap: make(map[string]*[]IMFriends.ImFriends),
	}
	mux sync.Mutex
)

// FriendCacheClient 定义了FriendCacheClient结构体
...
func (FCC *FriendCacheClient) SetFriendCache(uid string, friends *[]IMFriends.ImFriends) {
	// 加锁，保证并发安全
	mux.Lock()
	// 将好友缓存存入CacheMap
...
func (FCC *FriendCacheClient) GetFriendCache(uid string) (*[]IMFriends.ImFriends, error) {
	// 判断指定UID的好友缓存是否存在
	...
	Model.MYSQLDB.Table("im_friends").Where("m_id=?", uid).Find(&imFriendsList)
	// 将获取到的好友列表存入缓存
