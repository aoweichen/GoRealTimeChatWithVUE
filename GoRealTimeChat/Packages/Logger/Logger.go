...
func InitLogger() {
	filename := ConfigModels.ConfigData.Log.FileName
	maxBackup := ConfigModels.ConfigData.Log.MaxBackup
	maxSize := ConfigModels.ConfigData.Log.MaxSize
	maxAge := ConfigModels.ConfigData.Log.MaxAge
	compress := ConfigModels.ConfigData.Log.Compress
	logType := ConfigModels.ConfigData.Log.Type
	level := ConfigModels.ConfigData.Log.Level

	// 调用getWriter函数，传入filename、maxSize、maxBackup、maxAge、compress和logType等参数，返回一个writeSyncer对象，用于写入日志数据
	writeSyncer := getWriter(filename, maxSize, maxBackup, maxAge, compress, logType)
...
func getWriter(logFilePath string, maxSize, maxBackup, maxAge int, compress bool, logType string) zapcore.WriteSyncer {
	...
	// 判断当前环境是否为本地开发环境
	if ConfigModels.IsLocal() {
		// 如果是本地开发环境，则同时将日志输出到标准输出和日志文件中
