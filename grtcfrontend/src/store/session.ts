  ...
  actions: {
    ...
    async changeSessionPoint(message: chatItemType) {
      const idx: number = this.sessionList.findIndex((item) => {
        ...
        const res =
          item.to_id === message.to_id && item.from_id === userStore.userInfo.id
        return (
          res ||
          (item.to_id === message.from_id && item.from_id === message.to_id)
        )
      })
      // 有会话记录
      if (idx >= 0) {
        ...
        if (
          this.sessionList[idx].to_id === message.from_id &&
          this.sessionList[idx].from_id === message.to_id
        ) {
          // 判断是不是选中的会话
          if (
            !this.selectSession ||
    ...
    async setChattingRecords(session: sessionType<userType, groupType>) {
      ...
      if(session.channel_type === 1){
        ...
        const isArray = message.list instanceof Array
        let list:{ is_read: number; msg: string; data: string; from_id: number; created_at: string; msg_type: number; to_id: number; Users: { avatar: string; email: string; id: number; name: string }; id: number; channel_type: number; status: number }[] = []
        if(isArray){
    ...
    changeChattingRecords(message: chatItemType) {
      if (this.selectSession && this.chattingRecords) {
        ...
        if(message.channel_type === 1){
          // 接收消息
          if (
            message.from_id === this.selectSession.to_id &&
            message.to_id === this.selectSession.from_id
          ) {
        ...
        if(message.channel_type === 2){
          ...
          // 接收用户加入消息
          if(message.from_id === this.selectSession.group_id && message.msg_type === 6){
            this.chattingRecords.list.push(message)
            setStorage('chattingRecords', this.chattingRecords)
          }
