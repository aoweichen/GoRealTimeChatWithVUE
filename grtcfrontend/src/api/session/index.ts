...
export function sessionList(params?: Object) {
  return request.get<sessionType<userType, groupType>[]>('/sessions/get_list', {
    params,
  })
}
//创建会话
export function createSession(data: sessionData) {
  return request.post<sessionType<userType, groupType>>('/sessions/add', data)
}
// 移除会话
export function removeSession(id: number) {
  return request.delete(`/sessions/delete/${id}`)
}
